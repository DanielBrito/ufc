...
Lista* uniaoDeConjuntos(Lista* lA, Lista* lB){
	...
	for(p=lA; p!=NULL; p=p->prox){
		...
		lC = insereOrdenado(lC, p->info);
	}

	for(p=lB; p!=NULL; p=p->prox){

		lC = insereOrdenado(lC, p->info);
	}

	lC = removeElementosRepetidos(lC);

	return lC;
}


// QUESTÃO 16 - b (PROBLEMAS NA APRESENTACAO DO RESULTADO)
...
Lista* pertinenciaDeConjuntos(Lista* lA, Lista* lB){
