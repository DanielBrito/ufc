...
public class BMHS {
	...
	public static void main(String[] args) throws IOException {
		...
		for(String palavra : palavras) {
			...
			ocorrencias.add(new Palavra(palavra, bmhs(texto, palavra)));
		}
	...
	static int bmhs(String texto, String padrao) {
		...
		for(j=0; j<NUM_CARACTERES; j++) {
			...
			tabela[j] = tamanhoPadrao+1;			
		}
		
		for(j=0; j<tamanhoPadrao; j++) {
			
			tabela[(int)padrao.charAt(j)] = tamanhoPadrao-j;
		}
		...
		while(i<tamanhoTexto-1) {
			...
			i = i+tabela[(int)texto.charAt(i+1)];
		}
